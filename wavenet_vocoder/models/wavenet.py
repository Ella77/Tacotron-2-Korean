...
from Utils.Utils import get_hop_size
...
class wavenet():
	...
	def __init__(self, hparams):
		...
		### making sure that number of layers are [n * stacks]
		assert hparams.layers % hparams.stacks == 0
		## calculate number of layers per stack
		layers_per_stack = hparams.layers // hparams.stacks
		## this variable check that input is scalar or matrix
		...
		with tf.variable_scope('input_convolution'):
			if self.scalar_input:
				#### if input is scalar then output channel of first convolution layer is 1
				self.first_conv = Conv1d1x1(1, hparams.residual_channels, name='input_convolution')
			else:
				#### if input is scalar then output channel of first convolution layer is output_channels (in hparams)
		...
		with tf.variable_scope('loss') as scope:
			if self.is_training:
				...
				if is_mulaw_quantize(self._hparams.input_type):
					...
					log('debug: wavenet.py line 411, self.y_hat_eval={}'.format(self.y_hat_eval))
					log('debug: wavenet.py line 411, self.y_eval={}'.format(self.y_eval))
					log('debug: wavenet.py line 411, self.eval_length={}'.format(self.eval_length))
					log('debug: wavenet.py line 411, self.eval_loss={}'.format(self.eval_loss))
				else:
					if self._hparams.out_channels == 2:
