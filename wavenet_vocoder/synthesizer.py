...
from Utils.Utils import save_wavenet_wav, get_hop_size
...
#todo:##################################################################################################################
#todo:########                                                                                 #########################
#todo:########            fix this function. hang on when synthezing wavenet model             #########################
#todo:########                                                                                 #########################
#todo:##################################################################################################################
	...
	def synthesize(self, mel_spectrograms, speaker_ids, basenames, out_dir, log_dir):
		...
		log('debug: synthesizer.py line 83, feed_dict={}'.format(feed_dict))
		log('debug: synthesizer.py line 84, self.model.y_hat={}'.format(self.model.y_hat))
		...
		generated_wavs = self.session.run(self.model.y_hat, feed_dict=feed_dict) #### todo: problem here <<<<<<==========



		log('debug: synthesizer.py line 86, generated_wavs1={}'.format(generated_wavs))
		generated_wavs = [generated_wav[:length] for generated_wav, length in zip(generated_wavs, audio_lengths)]
		log('debug: synthesizer.py line 88, generated_wavs2={}'.format(generated_wavs))
		audio_filenames = []
		for i, generated_wav in enumerate(generated_wavs):
			#Save wav to disk
			audio_filename = os.path.join(out_dir, 'wavenet-audio-{}.wav'.format(basenames[i]))
			log('debug: synthesizer.py line 93, audio_filename={}'.format(audio_filename))
			...
			if log_dir is not None:
				...
				log('debug: synthesizer.py line 99, plot_filename={}'.format(plot_filename))
