...
from Utils.Utils import is_mulaw_quantize, is_scalar_input, trim_silence, get_hop_size, is_mulaw
...
class Feeder:
		...
		if local_condition:
			...
			for b in batch:
				...
				if max_time_steps is not None:
					max_steps = _ensure_divisible(max_time_steps, get_hop_size(self._hparams), True)
					if len(x) > max_time_steps:
						max_time_frames = max_steps // get_hop_size(self._hparams)
						start = np.random.randint(0, len(c) - max_time_frames)
						time_start = start * get_hop_size(self._hparams)
						x = x[time_start: time_start + max_time_frames * get_hop_size(self._hparams)]
	...
	def _assert_ready_for_upsample(self, x, c):
		assert len(x) % len(c) == 0 and len(x) // len(c) == get_hop_size(self._hparams)
